// src/pages/api/pedidos/pedidos.ts
...
const handler = nc<PedidoApiRequest, NextApiResponse<RespostaPadraoMsg | any>>()
  ...
  .get(async (req, res) => {
    try {
      ...
      const pedidos = (await PedidoModel.find(filtro).sort({
        criadoEm: -1,
      })) as IPedido[];
      ...
      for (let pedido of pedidos) {
        if (
          ...
          new Date(pedido.criadoEm).getTime() <
  ...
  .put(async (req, res) => {
    try {
      ...
      if (typeof enviado !== 'boolean') {
        return res
          .status(400)
          .json({ erro: 'Campo "enviado" (boolean) é obrigatório' });
      }
      ...
      if (upd.modifiedCount === 0) {
        return res
          .status(404)
          .json({ erro: 'Pedido não encontrado ou sem alterações' });
      }
